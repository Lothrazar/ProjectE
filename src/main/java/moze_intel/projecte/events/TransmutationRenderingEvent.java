...
public class TransmutationRenderingEvent {
  ...
  public static void onOverlay(DrawHighlightEvent.HighlightBlock event) {
    ...
    if (!enabled || stack.isEmpty() || !stack.isEmpty()) {
      transmutationResult = null;
      return;
    }
    RayTraceResult mop = null;//((PhilosophersStone) stack.getItem()).getHitBlock(player);
    if (mop instanceof BlockRayTraceResult) {
      ...
      if (transmutationResult != null) {
        ...
        //        list = PhilosophersStone.getAffectedPositions(world, rtr.getPos(), player, rtr.getFace(), mode, charge);
